...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["audio/*", "application/json"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for user-assets" ON storage.objects FOR SELECT USING (bucket_id = 'user-assets');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for user-assets" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'user-assets');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for user-assets" ON storage.objects FOR UPDATE USING (bucket_id = 'user-assets');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for user-assets" ON storage.objects FOR DELETE USING (bucket_id = 'user-assets');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'user-assets',
        public: true,
        allowed_mime_types: ["audio/*", "application/json"],
